package Interfaces;
/*
 * ѕрограмма предназначен€ дл€ делеени€ и отображени€ дес€тичных дробей
 *  (если заработает)
 */
public class DefaultFractionNumber implements FractionNumber {
	...
	public FractionNumber add(FractionNumber secondArgument) {
		int divident = this.getDividend() * secondArgument.getDividend() * this.getDivisor();
	...
	public FractionNumber sub(FractionNumber secondArgument) {
		int divident = this.getDividend() * secondArgument.getDivisor()
				- secondArgument.getDividend() * this.getDivisor();
		return new DefaultFractionNumber(divident, this.getDivisor() * secondArgument.getDivisor());
	}

	@Override
	public FractionNumber mul(FractionNumber secondArgument) {
		return new DefaultFractionNumber(this.getDividend() * secondArgument.getDividend(),
	...
	public FractionNumber div(FractionNumber secondArgument) {
		if (secondArgument.getDividend() == 0) {
			System.out.println("SecondArgument is 0. / by zero");
			return null;
		} else {
			return new DefaultFractionNumber(this.getDividend() * secondArgument.getDivisor(),
